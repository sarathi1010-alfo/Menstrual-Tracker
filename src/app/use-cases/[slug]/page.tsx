...
export default async function UseCasePost({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getArticleBySlug(slug);
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://lunacycle.alfo.online/use-cases/${article.meta.slug}`,
    },
  };
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-8">
      ...
      <header className="mb-8">
        <div className="text-sm font-semibold text-emerald-600 tracking-wide uppercase mb-2">Use Case Guide</div>
      ...
      {article.meta.takeaways && article.meta.takeaways.length > 0 && (
        <div className="bg-emerald-50 rounded-lg p-6 mb-8 border border-emerald-100">
          <h2 className="text-lg font-bold text-emerald-900 mb-3">Quick Reference</h2>
          <ul className="list-disc pl-5 space-y-2">
            {article.meta.takeaways.map((point, i) => (
              <li key={i} className="text-emerald-800">{point}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      <div className="prose prose-emerald prose-lg max-w-none mt-8">
        <MDXRemote
