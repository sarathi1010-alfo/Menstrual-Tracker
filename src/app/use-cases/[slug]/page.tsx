...
export async function generateMetadata({ params }: Props) {
  ...
  if (!article || article.meta.category !== 'use-cases') return { title: 'Not Found' };
...
export default async function UseCaseArticlePage({ params }: Props) {
  const resolvedParams = await params;
  const article = getArticleBySlug(resolvedParams.slug);
  ...
  return (
    <article className="max-w-3xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      ...
      <header className="mb-10">
        <p className="text-sm font-semibold tracking-wider text-primary uppercase mb-3">Use Case Guide</p>
