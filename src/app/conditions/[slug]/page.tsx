...
export async function generateStaticParams() {
  ...
  return articles.filter(a => a.category === 'conditions').map((article) => ({
    slug: article.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    alternates: {
      canonical: absoluteUrl(`/conditions/${resolvedParams.slug}`),
    }
  };
}
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": absoluteUrl(`/conditions/${resolvedParams.slug}`)
    }
  };
