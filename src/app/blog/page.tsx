...
export const metadata: Metadata = {
  title: 'LunaCycle Blog - Cycle Health Guides & Education',
    ...
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {articles.map((article) => {
          // Determine the correct route based on the slug.
          // This is a simplified way to handle different URL structures for these articles.
          let href = `/blog/${article.slug}`;
          if (article.slug.startsWith('what-is-')) {
             href = `/${article.slug}`;
          } else if (article.slug.includes('use-case') || article.slug.includes('teens-guide')) {
             href = `/use-cases/${article.slug}`;
          } else if (article.slug.includes('conditions') || article.slug.includes('pcos')) {
