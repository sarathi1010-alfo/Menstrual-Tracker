...
export default function BlogIndexPage() {
  ...
  // Exclude what-is from the main blog feed if you want, but for now we list everything,
  // or primarily 'cluster'
  const clusterArticles = allArticles.filter(a => a.category === 'cluster');
  const useCaseArticles = allArticles.filter(a => a.category === 'use-cases');
  const conditionsArticles = allArticles.filter(a => a.category === 'conditions');

  // We could just combine them for display in the blog index
  const articlesToDisplay = [...clusterArticles, ...useCaseArticles, ...conditionsArticles];
    ...
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {articlesToDisplay.map((article) => (
          <Link key={article.slug} href={`/${article.category === 'use-cases' ? 'use-cases/' : article.category === 'conditions' ? 'conditions/' : 'blog/'}${article.slug}`} className="group block">
