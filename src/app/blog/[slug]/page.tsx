...
export async function generateStaticParams() {
  ...
  return articles
    .filter(article => article.category === 'cluster')
    .map((article) => ({
      slug: article.slug,
    }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!article || article.meta.category !== 'cluster') {
    return {
      ...
      <article className="lg:w-2/3">
        ...
        <div className="mb-8">
          ...
          {article.meta.takeaways && article.meta.takeaways.length > 0 && (
            <div className="p-5 rounded-xl bg-blue-50 border border-blue-200 dark:bg-blue-900/20 dark:border-blue-800 shadow-sm mb-6">
              <h2 className="text-lg font-bold text-blue-900 dark:text-blue-100 mb-3">Key Takeaways</h2>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
              {relatedArticles.map(related => (
                <Link key={related.slug} href={`/${related.category === 'what-is' ? '' : related.category === 'use-cases' ? 'use-cases/' : related.category === 'conditions' ? 'conditions/' : 'blog/'}${related.slug}`} className="block group">
