...
import { ArrowLeft, BookOpen, Share2, CheckCircle2 } from 'lucide-react';
      ...
      <article className="lg:w-2/3">
        ...
        <div className="mb-8">
          ...
          {article.meta.takeaways && article.meta.takeaways.length > 0 && (
            <div className="mb-10 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              <h2 className="text-xl font-bold mb-4 flex items-center text-[var(--foreground)]">
                Key Takeaways
              </h2>
              <ul className="space-y-3">
                {article.meta.takeaways.map((takeaway, index) => (
                  <li key={index} className="flex items-start">
                    <CheckCircle2 size={20} className="mr-3 text-[var(--primary)] flex-shrink-0 mt-0.5" />
                    <span className="text-[var(--foreground)] leading-relaxed">{takeaway}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="mt-12">
            ...
            <div className="space-y-4">
              {article.meta.faqs.map((faq, index) => (
                <div key={index} className="p-5 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
