...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (article.meta.faqs && article.meta.faqs.length > 0) {
    schemas.push({
      "@context": "https://schema.org",
      "@type": "FAQPage",
      "mainEntity": article.meta.faqs.map((faq: { question: string, answer: string }) => ({
        "@type": "Question",
        "name": faq.question,
        "acceptedAnswer": {
          "@type": "Answer",
          ...
          {article.meta.takeaways && article.meta.takeaways.length > 0 && (
            <div className="p-6 rounded-xl bg-[var(--primary)]/5 border border-[var(--primary)]/20 shadow-sm mb-10">
              ...
              <ul className="list-disc pl-5 space-y-2 text-[var(--muted)]">
                {article.meta.takeaways.map((takeaway: string, index: number) => (
        ...
        {article.meta.faqs && article.meta.faqs.length > 0 && (
          <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-800">
            ...
            <div className="space-y-4">
              {article.meta.faqs.map((faq: { question: string, answer: string }, index: number) => (
