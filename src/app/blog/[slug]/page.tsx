...
export async function generateStaticParams() {
  ...
  return articles
    .filter(
      (a) =>
        a.category !== 'what-is' &&
        a.category !== 'use-cases' &&
        a.category !== 'conditions'
    )
    .map((a) => ({
      slug: a.slug,
    }));
...
export async function generateMetadata({ params }: Props) {
  ...
  if (!article) {
    return constructMetadata({ title: 'Article Not Found', path: `/blog/${slug}` });
  }
...
export default async function BlogPostPage({ params }: Props) {
  ...
  if (!article || article.meta.category === 'what-is' || article.meta.category === 'use-cases' || article.meta.category === 'conditions') {
    notFound();
  }
