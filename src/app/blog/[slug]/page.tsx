...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!article || article.meta.category !== 'blog') {
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  return (
    <article className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="prose prose-slate dark:prose-invert prose-a:text-[var(--primary)] hover:prose-a:text-[var(--primary)]/80 prose-headings:text-[var(--foreground)] prose-strong:text-[var(--foreground)] max-w-none">
        <MDXRemote
          source={content}
