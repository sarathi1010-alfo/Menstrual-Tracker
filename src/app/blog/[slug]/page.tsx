...
export async function generateStaticParams() {
  ...
  // Only generate routes for articles without an explicit category mapping
  // Explicit routes (use-cases, conditions, what-is) handle their own
  return slugs
    .filter((slug) => {
      const article = allArticles.find(a => a.slug === slug);
      if (!article) return false;
      const category = article.category || '';
      return category !== 'use-cases' && category !== 'conditions' && category !== 'what-is';
    })
    .map((slug) => ({
      slug: slug,
    }));
