      ...
      <article className="lg:w-2/3">
        ...
        <div className="mb-8">
          ...
          {article.meta.takeaways && article.meta.takeaways.length > 0 && (
            <div className="p-6 rounded-xl bg-[var(--primary)]/5 border border-[var(--primary)]/10 shadow-sm mb-10">
              <h2 className="text-sm font-bold text-[var(--primary)] uppercase tracking-wider mb-4 flex items-center">
                <CheckCircle2 size={16} className="mr-2" /> Key Takeaways
              </h2>
              <ul className="grid sm:grid-cols-1 gap-3 m-0 p-0 list-none">
                {article.meta.takeaways.map((takeaway, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-[var(--foreground)] m-0">
                    <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[var(--primary)]/20 text-[var(--primary)] flex items-center justify-center text-xs font-bold mt-0.5">
                      {idx + 1}
                    </span>
        ...
        {/* Medical Disclaimer */}
        <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border-l-4 border-amber-400 text-amber-800 dark:text-amber-200 text-sm mb-8 rounded-r-lg">
          <p className="m-0 font-medium">Medical Disclaimer:</p>
          <p className="m-0 opacity-90 italic">This information is for educational purposes only and does not constitute medical advice. Always consult with a qualified healthcare professional for medical decisions or if you have concerns about your health.</p>
        </div>
          ...
          <section className="mt-16 space-y-6">
            ...
            <div className="space-y-4">
              {article.meta.faqs.map((faq, idx) => (
                <div key={idx} className="card p-5 border border-gray-100 dark:border-gray-800">
