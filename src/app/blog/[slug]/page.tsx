          ...
          <div className="mb-10 p-6 rounded-2xl bg-[var(--primary)]/5 border border-[var(--primary)]/10">
            ...
            <ul className="grid gap-3 m-0 p-0 list-none">
              {article.meta.takeaways.map((takeaway, index) => (
                <li key={index} className="flex gap-3 text-[var(--foreground)] leading-snug">
                  <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-[var(--primary)] mt-2" />
                  {takeaway}
                </li>
              ))}
            </ul>
        ...
        {article.meta.faqs && article.meta.faqs.length > 0 && (
          <div className="mt-16 space-y-6">
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <HelpCircle size={24} className="text-[var(--secondary)]" />
              Frequently Asked Questions
            </h2>
            <div className="grid gap-4">
              {article.meta.faqs.map((faq, index) => (
                <div key={index} className="p-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-[var(--surface)]">
