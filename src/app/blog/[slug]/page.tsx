...
export async function generateStaticParams() {
  ...
  // Filter out explicit micro-answers and category routes from /blog/
  return allArticles
...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const allArticles = getAllArticles().filter(a => a.category !== 'what-is');
      ...
      <article className="lg:w-2/3">
        ...
        <div className="mb-8">
          ...
          {article.meta.takeaways && article.meta.takeaways.length > 0 && (
             <div className="p-5 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 shadow-sm mt-6 mb-10">
           ...
           <div className="mt-12">
             ...
             <div className="space-y-4">
                {article.meta.faqs.map((faq, index) => (
                   <details key={index} className="group border border-gray-200 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-gray-800/20">
                     <summary className="flex items-center justify-between p-4 font-semibold cursor-pointer">
                       {faq.question}
                     </summary>
                     <div className="p-4 pt-0 text-[var(--muted)] border-t border-gray-200 dark:border-gray-800 mt-2">
                       {faq.answer}
                     </div>
                   </details>
                ))}
